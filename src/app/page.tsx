...
import { DATA_UPDATED_LABEL } from "./site-data";
...
const averageLatency = Math.round(providers.reduce((total, provider) => total + provider.latency, 0) / providers.length);
const averageSpeed = Math.round(providers.reduce((total, provider) => total + provider.speed, 0) / providers.length);
  ...
  return (
    <main className="site-shell">
      <nav className="topbar">
        ...
        <div className="nav-links"><a href="#ranking">实时排名</a><a href="#reviews">每周评测</a><Link href="/wiki">机场百科</Link><a href="#archive">历史档案</a></div>
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-visual" aria-label="线路监测可视化">
          ...
          <div className="signal-card"><span>TEST COVERAGE</span><strong>{providers.length}</strong><small>家机场当前收录</small></div>
      ...
      <section className="home-articles" id="wiki-preview"><div className="section-heading"><div><p className="eyebrow">THE FIELD GUIDE</p><h2>机场百科</h2></div><Link className="text-link" href="/wiki">进入百科 ↗</Link></div><div className="home-article-grid"><Link href="/wiki/vpn-speed-test-report"><span>01 / 测速指标</span><strong>机场测速报告怎么看？哪些数据更值得比较？</strong><small>先看条件、样本和趋势，再看速度峰值。</small></Link><Link href="/wiki/packet-loss-and-reconnect"><span>02 / 测试方法</span><strong>为什么速度快，实际使用还是会卡？</strong><small>理解丢包、重连和持续连接稳定性。</small></Link><Link href="/wiki/airport-test-location-guide"><span>03 / 运营商观察</span><strong>为什么同一节点在不同地区表现不同？</strong><small>把城市、运营商和测试时间写清楚。</small></Link></div></section>
